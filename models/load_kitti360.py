...
class Kitti360_Data:
    def __init__(self,conf):
        ...
        self.intrinsics_all = np.array([
            [self.focal, 0, K[0][2], 0],
            [0, self.focal, K[1][2], 0],
            [0, 0, 1, 0],
            [0, 0, 0, 1]
        ])
        print(f'相机内参数：{self.intrinsics_all}')
    ...
    def gen_rays_novel_pose(self,pose):
        l = 1
        tx = torch.linspace(0, self.W - 1, self.W // l)
        ty = torch.linspace(0, self.H - 1, self.H // l)
        pixels_x, pixels_y = torch.meshgrid(tx, ty)
        p = torch.stack([pixels_x, pixels_y, torch.ones_like(pixels_y)], dim=-1)  # W, H, 3
        p = torch.matmul(self.intrinsics_inv[0, None, None, :3, :3],
                         p[:, :, :, None]).squeeze()  # W, H, 3
        rays_v = p / torch.linalg.norm(p, ord=2, dim=-1, keepdim=True)  # W, H, 3
